...
export class TeletrackerApi {
    ...
    async updateListTracking(thingId: number, addToLists: string[], removeFromLists: string[]) {
        return this.withTokenCheck(async () => {
    ...
    async addItemToList(listId: string, itemId: string) {
        ...
        return this.api.put<any>(`/api/v1/users/self/lists/${listId}`, { itemId }, { headers: this.authHeaders() });
    }
