...
/**
  ...
  */
class ProjectedReloadableConfig[T, U](
  reloadableConfig: ReloadableConfig[T]
)(
  f: T => U)
    extends ReloadableConfig[U] {
  override def watch(duration: Duration)(onChange: (U) => Unit) =
    reloadableConfig.watch(duration) { t =>
      onChange(f(t))
    }
