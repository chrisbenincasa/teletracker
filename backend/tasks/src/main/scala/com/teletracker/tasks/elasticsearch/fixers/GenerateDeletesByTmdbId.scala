...
import com.teletracker.tasks.model.{EsBulkDelete, EsItemDumpRow}
...
class GenerateDeletesByTmdbId @Inject()(
      ...
      .foreach(source => {
        try {
          source
            .getLines()
            .toStream
            .filter(tmdbIdsToId.isDefinedAt)
            .safeTake(limit)
            .foreach(row => {
              val itemId = tmdbIdsToId(row)
              rotater.writeLines(
                EsBulkDelete(
