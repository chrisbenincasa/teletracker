...
export default function Explore(props: Props) {
  ...
  const previousRouterPath = usePrevious(router.pathname);
  ...
  const loading = useStateSelector(state => state.popular.loadingPopular);
  ...
  const updateFeaturedItems = useCallback(() => {
    ...
    if (
      ...
      setFeaturedItems(
        featuredIndexes.map(index => thingsById[popular![index]]),
      );
      setNeedsNewFeatured(false);
    }
  ...
  const [handleRouteChangeComplete] = useDebouncedCallback((newUrl: string) => {
    console.log('route complete', newUrl);
    setNavigating(false);
  }, 100);

  const [handleRouteChangeStart] = useDebouncedCallback((newUrl: string) => {
    console.log('route started', newUrl);
    setNavigating(true);
  }, 100);
  ...
  useEffect(() => {
    ...
    const isNewFetch = popular && previousPopular && !loading;
  ...
  console.log(!navigating && !loading, `${!navigating} && ${!loading}`);

  return popular ? (
    <Fade in={!navigating && !loading} timeout={500}>
