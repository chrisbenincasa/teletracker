package com.chrisbenincasa.services.teletracker.auth.jwt
...
class JwtVendor @Inject()(config: TeletrackerConfig) {
  def vend(email: String): String = {
    ...
    if (config.auth.jwt.expiration.isDefined) {
      jwt.setExpiration(new java.util.Date(System.currentTimeMillis() + config.auth.jwt.expiration.get.toMillis))
    }
