...
class GenreReferences @Inject()(
  ...
  class GenreReferencesTable(tag: Tag) extends Table[GenreReference](tag, "genre_references") {
    ...
    def source_id_genre_unique = index("genre_references_source_id_genre_unique", (externalSource, externalId, genreId), true)

    def genre = foreignKey("network_references_network_id_fk", genreId, genres.query)(_.id)
