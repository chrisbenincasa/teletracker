...
class ThingNetworks @Inject()(
  ...
  class ThingNetworksTable(tag: Tag) extends Table[ThingNetwork](tag, "thing_networks") {
    ...
    def fkThing = foreignKey("thing_networks_fk_things", thingId, things.query)(_.id)
    def fkNetwork = foreignKey("thing_networks_fk_networks", networkId, networks.query)(_.id)
