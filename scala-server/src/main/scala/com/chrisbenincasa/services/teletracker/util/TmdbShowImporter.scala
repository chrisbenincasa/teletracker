...
import com.chrisbenincasa.services.teletracker.model.justwatch._
...
class TmdbShowImporter @Inject()(
  ...
  def handleShows(shows: List[tmdb.TvShow], pullSeasons: Boolean, pullAvailability: Boolean) = {
    ...
    SequentialFutures.serialize(shows, Some(250 millis))(show => {
      ...
      val seasonsFut = if (pullSeasons) {
        for {
          ...
          seasonsAndEpisodes <- saveSeasons(show, thing, networksBySource, pullAvailability)
      ...
      for {
        ...
        networksBySource <- allNetworks
        _ <- if (pullAvailability) saveAvailability(show, seasonsAndEpisodes, networksBySource) else Future.unit
      } yield savedShow
    })
  }
  ...
  private def saveSeasons(
    ...
    tvShowDbAccess.findAllSeasonsForShow(entity.id.get).flatMap(existingSeasons => {
      SequentialFutures.serialize(show.seasons.getOrElse(Nil), Some(250 millis))(season => {
        logger.info(s"Handling season ID = ${season.id}")

        val seasonFut = tmdbClient.makeRequest[tmdb.TvShowSeason](
        ...
        seasonFut.flatMap(foundSeason => {
          ...
          val seasonSaveFut = existingSeasons.find(_.number == foundSeason.season_number.get) match {
            case None =>
              logger.info("Saving TV show season")
              val newSeason = TvShowSeason(None, foundSeason.season_number.get, entity.id.get, foundSeason.overview, foundSeason.air_date.map(new LocalDate(_)))
              tvShowDbAccess.insertSeason(newSeason)

            case Some(s) => Future.successful(s)
          }

          val episodeSaves = foundSeason.episodes.getOrElse(Nil).map(episode => {
            tvShowDbAccess.findEpisodeByExternalId(ExternalSource.TheMovieDb, episode.id.toString).flatMap {
              case None =>
                seasonSaveFut.flatMap(s => {
                  val newEpisode = TvShowEpisode(None, episode.episode_number.get, entity.id.get, s.id.get, episode.name.get, episode.production_code)
                  tvShowDbAccess.insertEpisode(newEpisode).flatMap(savedEpisode => {
                    tmdbEntityProcessor.handleExternalIds(Right(savedEpisode), None, Some(episode.id.toString)).map(_ => savedEpisode)
                  })
                })
  ...
  private def saveAvailability(
    ...
    val episodesByNumber = tvShowSeasonsAndEpisodes.flatMap {
      case (season, episodes) => episodes.map(ep => (season.number, ep.number) -> ep)
    }.toMap

    justWatchResFut.flatMap(justWatchRes => {
      val availabilitiesFut = matchJustWatchShow(show, justWatchRes.items).map {
        case matchedItem =>
          for {
            jwShow <- justWatchClient.makeRequest[JustWatchShow](s"/content/titles/show/${matchedItem.id}/locale/en_US")
            jwPartialSeasons = jwShow.seasons.getOrElse(Nil)
            jwSeasonFuts = jwPartialSeasons.map(jwSeason => {
              justWatchClient.makeRequest[JustWatchSeason](s"/content/titles/show_season/${jwSeason.id}/locale/en_US")
            })
            ...
            for {
              ...
              provider <- networksBySource.get(ExternalSource.JustWatch -> jwOffer.provider_id.toString).toList
            } yield {
  ...
  private def matchJustWatchShow(show: tmdb.TvShow, popularItems: List[PopularItem]): Option[PopularItem] = {
    popularItems.find(item => {
      ...
      val originalMatch = show.original_name.exists(og => item.original_title.contains(og))
