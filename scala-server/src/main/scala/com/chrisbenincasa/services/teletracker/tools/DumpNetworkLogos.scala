...
import com.chrisbenincasa.services.teletracker.model.tmdb.Network
...
object DumpNetworkLogos extends com.twitter.inject.app.App {
  ...
  override protected def run(): Unit = {
    ...
    lazy val imagesClient = Http.client.withTls("image.tmdb.org").newService("image.tmdb.org:443")
    ...
    val lines = scala.io.Source.fromFile(new File(System.getProperty("user.dir") + "/data/tv_network_ids_06_26_2018.txt")).getLines()
    ...
    val finish = SequentialFutures.serialize(parsed, Some(250 millis)) {
      case Left(error) =>
        ...
        Future.unit
      case Right(line) =>
        client.makeRequest[Network](s"/network/${line.id}", Seq("append_to_response" -> "images")).flatMap(network => {
          val logos = network.images.flatMap(_.logos).getOrElse(Nil)
          println(s"Found ${logos.size} logos for network ${network.name.get}")
          val requests = for {
            ...
            () => (imagesClient(r): Future[Response]).map(response => {
              val f = new File(outputDirBase + s"/${network.name.get}/${network.name.get}-${idx}_${size}.$typ")
          ...
          SequentialFutures.batchedIterator[() => Future[Unit], Unit](requests.iterator, 5, _ ++ _)(batch => {
...
case class NetworkLine(id: Int, name: String)
