...
import com.chrisbenincasa.services.teletracker.db.model.{Certification, CertificationType, Certifications}
...
import com.chrisbenincasa.services.teletracker.model.tmdb.CertificationListResponse
...
class CertificationSeeder @Inject()(tmdbClient: TmdbClient, provider: DbProvider, certifications: Certifications) {
  import certifications.driver.api._

  def run() = {
    val movieCerts = Await.result(tmdbClient.makeRequest[CertificationListResponse]("certification/movie/list"), Duration.Inf)
    ...
    val inserts = movieCerts.certifications.flatMap {
      case (region, certs) =>
        certs.map(cert => {
          val certModel = Certification(None, CertificationType.Movie, region, cert.certification, cert.meaning, cert.order)
    ...
    val tvCerts = Await.result(tmdbClient.makeRequest[CertificationListResponse]("certification/tv/list"), Duration.Inf)

    val tvInserts = tvCerts.certifications.flatMap {
      case (region, certs) =>
        certs.map(cert => {
          val certModel = Certification(None, CertificationType.Tv, region, cert.certification, cert.meaning, cert.order)
          certifications.query += certModel
        })
    }
