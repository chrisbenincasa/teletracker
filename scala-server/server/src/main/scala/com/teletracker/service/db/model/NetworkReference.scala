...
class NetworkReferences @Inject()(
  ...
  class NetworkReferencesTable(tag: Tag) extends Table[NetworkReference](tag, "network_references") {
    ...
    def source_id_network_unique = index("network_references_source_id_network_unique", (externalSource, externalId, networkId), true)

    def networkId_fk = foreignKey("network_references_network_id_fk", networkId, networks.query)(_.id)
